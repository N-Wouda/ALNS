...
def test_accepts_below_threshold():
    ...
    adaptive_threshold(rnd.default_rng(), One(), One(), One())
    adaptive_threshold(rnd.default_rng(), One(), One(), Zero())

    # The threshold is set at 0 + 0.5 * (0.5 - 0) = 0.25
    assert_(adaptive_threshold(rnd.default_rng(), One(), One(), Zero()))


def test_rejects_above_threshold():
    adaptive_threshold = AdaptiveThreshold(eta=0.5, gamma=4)
    adaptive_threshold(rnd.default_rng(), One(), One(), Two())
    adaptive_threshold(rnd.default_rng(), One(), One(), Zero())

    # The threshold is set at 0 + 0.5 * (1 - 0) = 0.5
    assert_(not adaptive_threshold(rnd.default_rng(), One(), One(), One()))


def test_accepts_equal_threshold():
    accept = AdaptiveThreshold(eta=0.5, gamma=4)
    accept(rnd.default_rng(), One(), One(), VarObj(7100))
    accept(rnd.default_rng(), One(), One(), VarObj(7200))

    # The threshold is set at 7100 + 0.5 * (7140 - 7100) = 7120
    assert_(accept(rnd.default_rng(), One(), One(), VarObj(7120)))
...
def test_accepts_over_gamma_candidates():
    ...
    assert_(accept(rnd.default_rng(), One(), One(), VarObj(7000)))
...
def test_rejects_over_gamma_candidates():
    ...
    for value in [7100, 7200, 7200, 7000]:
        accept(rnd.default_rng(), One(), One(), VarObj(value))

    # The threshold is set at 7000 + 0.2 * (7100 - 7000) = 7020
    result = accept(rnd.default_rng(), One(), One(), VarObj(7100))
    assert_(not result)
...
def test_evaluate_consecutive_solutions():
    ...
    assert_(accept(rnd.default_rng(), One(), One(), VarObj(7100)))

    # The threshold is set at 7125, hence the solution is accepted.
    result = accept(rnd.default_rng(), One(), One(), VarObj(7200))
    assert_(not result)
...
def test_history():
    ...
    accept(rnd.default_rng(), One(), One(), VarObj(7120))
