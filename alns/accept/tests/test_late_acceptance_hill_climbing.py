...
def test_raises_invalid_n_last(n_last):
    with assert_raises(ValueError):
        LateAcceptanceHillClimbing(n_last)
...
def test_late_acceptance(n_last):
    ...
    that is better than the current solution n_last ago.
    """
    lahc = LateAcceptanceHillClimbing(n_last)
    ...
    # The current solution n_last ago has value 2, so the candidate
    # solution with value 1 should be accepted.
    assert_equal(lahc.last_objectives[0], 2)
...
def test_improved_late_acceptance(n_last):
    ...
    solution that 1) is better than the current solution n_last ago
    or 2) is better than the current solution.
    """
    ilahc = LateAcceptanceHillClimbing(n_last, improved=True)
